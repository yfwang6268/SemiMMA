...
#' @export
estimating_function <- function(param, subset_data){
...
#' @param outcome_paramete A vector with previous parameters
#' @param y A matrix with effect sizes 
#' @param s A matrix with standard deviations 
...
one_step_update <- function(outcome_parameter, y, s){
  ...
  for(j in 1:J){
    ...
    while(warn_msg > 0){
      ...
      temp_res <- estimating_function(init_phi, temp_subset_data)
      temp_res_cov <-  cov(temp_res)
      diag(temp_res_cov) <- diag(temp_res_cov) + 1e-6
      temp_gmm_res <- tryCatch(gmm::gmm(estimating_function, temp_subset_data, init_phi, 
  ...
  update_output_parameter = c(update_mu, update_tau, update_rho)
...
#' @param y A matrix with effect sizes 
#' @param s A matrix with standard deviations 
#' @return A matrix with estimates of effect sizes and standard deviations per row
#' @export
SemiMMA_fit <- function(y, s, J){
  J = ncol(y)
  ...
  init_para = c(init_mu, init_tau, init_rhoB)
...
#' Semiparametric Multivariate Meta-Analysis
#' 
#' Conducts a semiparametric multivariate meta-analysis to estimate effect sizes and standard deviations.
#' @param data A matrixwith effect sizes and standard deviations in alternating columns, one study for each row
#' @return A matrix with estimates of effect sizes and standard deviations for each outcome  per row
#' @export

SemiMMA <- function(input_data){
  ...
  full_miss_studies_index = rowSums(is.na(y)) == J
  y = y[!full_miss_studies_index,]
  s = s[!full_miss_studies_index,]
  n = n[!full_miss_studies_index]
  
  result <- NA
  
  for(j in 1:J){
    temp_missing_row <- is.na(s[,j])
    temp_k <- mean(1/s[!temp_missing_row,j]^2/n[!temp_missing_row])
